...
const supervisorEnv = "SABER_CONTROLLER_SUPERVISOR"

func pidFilePath() string {
	return filepath.Join("pids", "controller.pid")
}
...
// args from os.Executable() and os.Args[1:]). Called when SABER_CONTROLLER_SUPERVISOR=1.
...
func runRestart(cmd *cobra.Command, args []string) error {
	if err := runStop(cmd, args); err != nil && err.Error() != "controller not running" {
		return err
	}
